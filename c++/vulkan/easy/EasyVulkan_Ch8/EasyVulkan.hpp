...
const auto& CreateRpwf_Screen()
{
    ...
    rpwf.renderPass_.Create(renderPassCreateInfo);
    auto CreateFramebuffers = []
    {
        rpwf.framebuffers_.resize(graphicsBase::Base().SwapchainImageCount());
        VkFramebufferCreateInfo framebufferCreateInfo = {
            .renderPass      = rpwf.renderPass_,
            .attachmentCount = 1,
        ...
        for (size_t i = 0; i < graphicsBase::Base().SwapchainImageCount(); i++)
        {
            ...
            rpwf.framebuffers_[i].Create(framebufferCreateInfo);
        }
    };
    auto DestroyFramebuffers = []
    {
        rpwf.framebuffers_.clear();
    };
    CreateFramebuffers();
...
const auto& CreateRpwf_Canvas(VkExtent2D canvasSize)
{
    ...
    VkFramebufferCreateInfo framebufferCreateInfo = {
        ...
        .layers          = 1};
    rpwf.framebuffer_.Create(framebufferCreateInfo);
    return rpwf;
}

void CmdClearCanvas(VkCommandBuffer commandBuffer, VkClearColorValue clearColor)
{
    // Call this function before rpwf.renderPass_ begins.
...
public:
    ...
    texture2d operator()(const uint8_t* pImageData, VkExtent2D extent, VkFormat format_initial) const
    {
        ...
        {
            using texture::imageMemory_;
            using texture::imageView_;
        ...
        pTexture->imageMemory_.Create(imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        VkImage image = pTexture->imageMemory_.Image();
        // Create framebuffer
        pTexture->imageView_.Create(image, VK_IMAGE_VIEW_TYPE_2D, format_final, {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1});
        VkFramebufferCreateInfo framebufferCreateInfo = {
            .renderPass      = renderPass_,
            .attachmentCount = 1,
            .pAttachments    = pTexture->imageView_.Address(),
            .width           = extent.width,
        ...
        if (mipLevelCount > 1)
            pTexture->imageView_.~imageView(),
                pTexture->imageView_.Create(image, VK_IMAGE_VIEW_TYPE_2D, format_final, {VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevelCount, 0, 1});
        return texture;
    }
    ...
    void Instantiate(VkFormat format_final, bool generateMipmap, callback_copyData_t callback_copyMipLevel0)
    {
        ...
        pipelineCiPack.createInfo.renderPass = renderPass_;
        ...
        pipelineCiPack.scissors.emplace_back(VkRect2D{VkOffset2D{}, VkExtent2D{
        ...
        pipelineCiPack.colorBlendAttachmentStates.emplace_back(VkPipelineColorBlendAttachmentState{true,
        ...
        pipeline_.Create(pipelineCiPack);
    }
};
} // namespace easyVulkan