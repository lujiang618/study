...
public:
    ...
    const VkFormatProperties& FormatProperties(VkFormat format) const
    {
        ...
        if (uint32_t(format) >= std::size(formatInfos_v1_0))
            outStream << "[ FormatProperties ] ERROR This function only supports definite formats provided by VK_VERSION_1_0.",
                abort();
#endif
...
#ifndef NDEBUG
    if (uint32_t(format) >= std::size(formatInfos_v1_0))
        outStream << "[ FormatInfo ] ERROR This function only supports definite formats provided by VK_VERSION_1_0.",
            abort();
#endif
...
public:
    ...
    result_t Signal(uint64_t value = 0) const
    {
        ...
        if (result)
            outStream << "[ timelineSemaphore ] ERROR Failed to signal the semaphore!\nError code: {}" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t GetValue(uint64_t& value) const
    {
        ...
        if (result)
            outStream << "[ timelineSemaphore ] ERROR Failed to get the counter value of the semaphore!\nError code: {}" << int32_t(result) << std::endl;
        return result;
    }
    ...
    static result_t Wait(arrayRef<const timelineSemaphore> semaphores, arrayRef<uint64_t> values, bool waitAll = true)
    {
        if (semaphores.Count() != values.Count())
            if (semaphores.Count() < values.Count())
            {
                outStream << "[ timelineSemaphore ] ERROR For each semaphore, must provide a corresponding counter value!";
                return VK_RESULT_MAX_ENUM; // No proper VkResult enum value, don't use VK_ERROR_UNKNOWN
            }
            else
                outStream << "[ timelineSemaphore ] WARNING\nProvided counter values are more than semaphores!";
        ...
        if (result)
            outStream << "[ timelineSemaphore ] ERROR Failed to wait for semaphores!\nError code: {}" << int32_t(result) << std::endl;
        return result;
    }
};
...
private:
    ...
    {
        static stagingBuffer stagingBufferInstance;
        // Assuming graphicsBase is a valid class with a method AddCallback_DestroyDevice
        graphicsBase::Base().AddCallback_DestroyDevice([]
        {
            // Using the destructor through the proper scope
            std::unique_ptr<stagingBuffer> ptr(&stagingBufferInstance);
            // Destructor is automatically invoked when ptr goes out of scope
        });
        return &stagingBufferInstance;
    }

public:
    // Singleton pattern - Get will return the single instance
    static stagingBuffer& Get()
    {
        static stagingBuffer* pointer = Create();
        return *pointer;
    }

protected:
    bufferMemory bufferMemory_;
    VkDeviceSize memoryUsage_ = 0;
    image        aliasedImage_;
...
public:
    ...
    // Getter
    operator VkBuffer() const { return bufferMemory_.Buffer(); }

    const VkBuffer* Address() const { return bufferMemory_.AddressOfBuffer(); }

    VkDeviceSize AllocationSize() const { return bufferMemory_.AllocationSize(); }

    VkImage AliasedImage() const { return aliasedImage_; }

    // Const Function
    void RetrieveData(void* pData_src, VkDeviceSize size) const
    {
        bufferMemory_.RetrieveData(pData_src, size);
    }
    ...
    void Expand(VkDeviceSize size)
    {
        ...
        bufferMemory_.Create(bufferCreateInfo, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    }
    ...
    void* MapMemory(VkDeviceSize size)
    {
        ...
        bufferMemory_.MapMemory(pData_dst, size);
        memoryUsage_ = size;
        return pData_dst;
    }
    ...
    void UnmapMemory()
    {
        bufferMemory_.UnmapMemory(memoryUsage_);
        memoryUsage_ = 0;
    }
    ...
    {
        Expand(size);
        bufferMemory_.BufferData(pData_src, size);
    }
    ...
    VkImage AliasedImage2d(VkFormat format, VkExtent2D extent)
    {
        ...
        aliasedImage_.~image();
        aliasedImage_.Create(imageCreateInfo);
        VkImageSubresource  subResource       = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0};
        VkSubresourceLayout subresourceLayout = {};
        vkGetImageSubresourceLayout(graphicsBase::Base().Device(), aliasedImage_, &subResource, &subresourceLayout);
        if (subresourceLayout.size != imageDataSize)
            return VK_NULL_HANDLE; // No padding bytes
        aliasedImage_.BindMemory(bufferMemory_.Memory());
...
public:
    ...
    {
        if (bufferMemory_.MemoryProperties() & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
        {
            bufferMemory_.BufferData(pData_src, size, offset);
            return;
        }
        ...
        vkCmdCopyBuffer(commandBuffer, stagingBuffer::Buffer_MainThread(), bufferMemory_.Buffer(), 1, &region);
        commandBuffer.End();
    ...
    void TransferData(const void* pData_src, uint32_t elementCount, VkDeviceSize elementSize, VkDeviceSize stride_src, VkDeviceSize stride_dst, VkDeviceSize offset = 0) const
    {
        ...
        {
            void* pData_dst = nullptr;
            bufferMemory_.MapMemory(pData_dst, stride_dst * elementCount, offset);
            ...
            bufferMemory_.UnmapMemory(elementCount * stride_dst, offset);
            return;
        }
        ...
        vkCmdCopyBuffer(commandBuffer, stagingBuffer::Buffer_MainThread(), bufferMemory_.Buffer(), elementCount, regions.get());
        commandBuffer.End();
        graphicsBase::Plus().ExecuteCommandBuffer_Graphics(commandBuffer);
    }
    ...
    {
        vkCmdUpdateBuffer(commandBuffer, bufferMemory_.Buffer(), offset, size_Limited_to_65536, pData_src);
    }

    void CmdUpdateBuffer(VkCommandBuffer commandBuffer, const auto& data_src) const
    {
        vkCmdUpdateBuffer(commandBuffer, bufferMemory_.Buffer(), 0, sizeof data_src, &data_src);
    }
    ...
    void Create(VkDeviceSize size, VkBufferUsageFlags desiredUsages_Without_transfer_dst)
    {
        ...
        false ||
            bufferMemory_.CreateBuffer(bufferCreateInfo) ||
            bufferMemory_.AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) &&
                bufferMemory_.AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) ||
            bufferMemory_.BindMemory();
    }
...
public:
    ...
    VkImage Image() const { return imageMemory_.Image(); }

    const VkImageView* AddressOfImageView() const { return imageView_.Address(); }

    const VkImage* AddressOfImage() const { return imageMemory_.AddressOfImage(); }

    // Const Function
    VkDescriptorImageInfo DescriptorImageInfo(VkSampler sampler) const
    {
        return {sampler, imageView_, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
    }
};
...
protected:
    ...
    void CreateImageMemory(VkImageType imageType, VkFormat format, VkExtent3D extent, uint32_t mipLevelCount, uint32_t arrayLayerCount, VkImageCreateFlags flags = 0)
    {
        ...
        imageMemory_.Create(imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    }
    ...
    {
        imageView_.Create(imageMemory_.Image(), viewType, format, {VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevelCount, 0, arrayLayerCount}, flags);
    }
    ...
    static std::unique_ptr<uint8_t[]> LoadFile_Internal(const auto* address, size_t fileSize, VkExtent2D& extent, formatInfo requiredFormatInfo)
    {
        ...
        if (!(requiredFormatInfo.rawDataType == formatInfo::floatingPoint && requiredFormatInfo.sizePerComponent == 4) &&
            ...
            outStream << "[ texture ] ERROR Required format is not available for source image data!",
                abort();
#endif
        ...
        if constexpr (std::is_same_v<decltype(address), const char*>)
        {
            ...
            if (!pImageData)
                outStream << "[ texture ] ERROR Failed to load the file: {}" << address << std::endl;
        }
        if constexpr (std::is_same_v<decltype(address), const uint8_t*>)
        {
            if (fileSize > INT32_MAX)
            {
                outStream << "[ texture ] ERROR Failed to load image data from the given address! Data size must be less than 2G!";
                return {};
            }
            if (requiredFormatInfo.rawDataType == formatInfo::integer)
                if (requiredFormatInfo.sizePerComponent == 1)
            ...
            if (!pImageData)
                outStream << "[ texture ] ERROR Failed to load image data from the given address!";
        }
    ...
    static bool CheckArguments(VkImageType imageType, VkExtent3D extent, uint32_t arrayLayerCount, VkFormat format_initial, VkFormat format_final, bool generateMipmap)
    {
        ...
        if (FormatProperties(format_final).optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT)
        {
            ...
            if (format_initial == format_final)
                return FormatProperties(format_final).optimalTilingFeatures & VK_FORMAT_FEATURE_TRANSFER_DST_BIT &&
            ...
            // Case: Copy data from staging buffer to initial image, then blit initial image to final image
                ...
                return FormatProperties(format_initial).optimalTilingFeatures & (VK_FORMAT_FEATURE_BLIT_SRC_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT) &&
...
protected:
    ...
    void Create_Internal(VkFormat format_initial, VkFormat format_final, bool generateMipmap)
    {
        ...
        if (format_initial == format_final)
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_.Image(), imageMemory_.Image(), extent, mipLevelCount, 1);
        else if (VkImage image_conversion = stagingBuffer::AliasedImage2d_MainThread(format_initial, extent))
            BlitAndGenerateMipmap2d(image_conversion, imageMemory_.Image(), extent, mipLevelCount, 1);
        else
        {
            VkImageCreateInfo imageCreateInfo = {
                .imageType   = VK_IMAGE_TYPE_2D,
            ...
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_conversion.Image(), imageMemory_.Image(), extent, mipLevelCount, 1);
        }
    }
...
protected:
    ...
    void Create_Internal(VkFormat format_initial, VkFormat format_final, bool generateMipmap)
    {
        ...
        // Copy data and generate mipmap
        if (format_initial == format_final)
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_.Image(), imageMemory_.Image(), extent, mipLevelCount, layerCount);
        else
        {
            VkImageCreateInfo imageCreateInfo = {
                .imageType   = VK_IMAGE_TYPE_2D,
                ...
                .usage       = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT};
            vulkan::imageMemory imageMemory_conversion(imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_conversion.Image(), imageMemory_.Image(), extent, mipLevelCount, layerCount);
        }
    }
...
public:
    ...
    void Create(const char* filepath, VkExtent2D extentInTiles, VkFormat format_initial, VkFormat format_final, bool generateMipmap = true)
    {
        ...
        {
            outStream << "[ texture2dArray ] ERROR Layer count is out of limit! Must be less than: {}\nFile: {}" << graphicsBase::Base().PhysicalDeviceProperties().limits.maxImageArrayLayers << filepath;
            return;
        }
        ...
        if (pImageData)
            if (fullExtent.width % extentInTiles.width ||
                ...
                outStream << "[ texture2dArray ] ERROR Image not available!\nFile: " << filepath << "\nImage width should be in multiples of " << extentInTiles.width << "\nImage height should be in multiples of" << extentInTiles.width << std::endl; // fallthrough
            else
    ...
    void Create(const uint8_t* pImageData, VkExtent2D fullExtent, VkExtent2D extentInTiles, VkFormat format_initial, VkFormat format_final, bool generateMipmap = true)
    {
        ...
        {
            outStream << "[ texture2dArray ] ERROR Layer count is out of limit! Must be less than: {}" << graphicsBase::Base().PhysicalDeviceProperties().limits.maxImageArrayLayers;
            return;
        }
        if (fullExtent.width % extentInTiles.width ||
            fullExtent.height % extentInTiles.height)
        {
            outStream << "[ texture2dArray ] ERROR Image not available!\nImage width should be in multiples of " << extentInTiles.width << "\nImage height should be in multiples of {}" << extentInTiles.height << std::endl;
            return;
        }
        ...
        for (size_t i = 0; i < filepaths.Count(); i++)
        {
            ...
            if (psImageData[i])
            {
                ...
                if (extent.width == extent_currentLayer.width &&
                    ...
                    outStream << "[ texture2dArray ] ERROR Image not available!\nFile: {}\nAll the images must be in same size!" << filepaths[i]; // fallthrough
            }
            return;
        }
...
protected:
    ...
    void Create_Internal(VkFormat format_initial, VkFormat format_final, bool generateMipmap)
    {
        ...
        // Copy data and generate mipmap
        if (format_initial == format_final)
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_.Image(), imageMemory_.Image(), extent, mipLevelCount, 6);
        else
        {
            VkImageCreateInfo imageCreateInfo = {
                ...
                .usage       = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT};
            vulkan::imageMemory imageMemory_conversion(imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            CopyBlitAndGenerateMipmap2d(stagingBuffer::Buffer_MainThread(), imageMemory_conversion.Image(), imageMemory_.Image(), extent, mipLevelCount, 6);
        }
    }
    ...
    void Create(const char* filepath, const glm::uvec2 facePositions[6], VkFormat format_initial, VkFormat format_final, bool lookFromOutside = false, bool generateMipmap = true)
    {
        ...
        if (pImageData)
            if (VkExtent2D extentInTiles = GetExtentInTiles(facePositions, lookFromOutside);
                ...
                fullExtent.height % extentInTiles.height)
                outStream << "[ textureCube ] ERROR Image not available!\nFile: " << filepath << "nImage width should be in multiples of " << extentInTiles.width << "\nImage height should be in multiples of {}" << extentInTiles.width << std::endl;
            else
            {
                extent.width  = fullExtent.width / extentInTiles.width;
                extent.height = fullExtent.height / extentInTiles.height;
                    ...
                    for (uint32_t j = 0; j < extent.height; j++)
                        for (uint32_t k = 0; k < extent.width; k++)
                            memcpy(
                                ...
                                dataSizePerPixel),
                                pData_dst += dataSizePerPixel;
        else if (extentInTiles.width == 1 && extentInTiles.height == 6 &&
...
public:
    ...
    // Getter
    operator VkQueryPool() const { return queryPool_; }

    const VkQueryPool* Address() const { return queryPool_.Address(); }
    ...
    {
        queryPool_.CmdCopyResults(commandBuffer, firstQueryIndex, queryCount, buffer_dst, offset_dst, stride, VK_QUERY_RESULT_WAIT_BIT);
    }
    ...
    void Create(uint32_t capacity)
    {
        ...
        queryPool_.Create(VK_QUERY_TYPE_OCCLUSION, Capacity());
    }
    ...
    {
        return queryPool_.GetResults(0, queryCount, queryCount * 4, passingSampleCounts.data(), 4);
    }
};
...
public:
    ...
    void CmdReset(VkCommandBuffer commandBuffer) const
    {
        queryPool_.CmdReset(commandBuffer, 0, 1);
    }

    void CmdBegin(VkCommandBuffer commandBuffer) const
    {
        queryPool_.CmdBegin(commandBuffer, 0);
    }

    void CmdEnd(VkCommandBuffer commandBuffer) const
    {
        queryPool_.CmdEnd(commandBuffer, 0);
    }
    ...
    void Create()
    {
        queryPool_.Create(VK_QUERY_TYPE_PIPELINE_STATISTICS, 1, (1 << statisticCount) - 1);
    }

    result_t GetResults()
    {
        return queryPool_.GetResults(0, 1, sizeof statistics, statistics, sizeof statistics);
    }
};
...
public:
    ...
    {
        queryPool_.CmdWriteTimestamp(commandBuffer, pipelineStage, queryIndex);
    }
    ...
    void Create(uint32_t capacity)
    {
        ...
        queryPool_.Create(VK_QUERY_TYPE_TIMESTAMP, Capacity());
    }
    ...
    result_t GetResults(uint32_t queryCount)
    {
        return queryPool_.GetResults(0, queryCount, queryCount * 4, timestamps.data(), 4);
    }
};
} // namespace vulkan