...
#define DefineHandleTypeOperator \
    operator volatile decltype(handle)() const { return handle; }
#else
...
class graphicsBase
{
    ...
    result_t GetQueueFamilyIndices(VkPhysicalDevice physicalDevice, bool enableGraphicsQueue, bool enableComputeQueue, uint32_t (&queueFamilyIndices)[3])
    {
        ...
        for (uint32_t i = 0; i < queueFamilyCount; i++)
        {
            ...
            if (surface)
                ...
                {
                    outStream << "[ graphicsBase ] ERROR Failed to determine if the queue family supports presentation!Error code:" << int32_t(result) << std::endl;
                    return result;
                }
    ...
    result_t CreateSwapchain_Internal()
    {
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to create a swapchain!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get the count of swapchain images!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        swapchainImages.resize(swapchainImageCount);
        if (VkResult result = vkGetSwapchainImagesKHR(device, swapchain, &swapchainImageCount, swapchainImages.data()))
        {
            outStream << "[ graphicsBase ] ERROR Failed to get swapchain images!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        ...
        for (size_t i = 0; i < swapchainImageCount; i++)
        {
            ...
            {
                outStream << "[ graphicsBase ] ERROR Failed to create a swapchain image view!Error code:" << int32_t(result) << std::endl;
                return result;
            }
        }
        return VK_SUCCESS;
    ...
    result_t CreateDebugMessenger()
    {
        ...
        {
            outStream << pCallbackData->pMessage;
            return VK_FALSE;
        };
        ...
        if (vkCreateDebugUtilsMessenger)
        {
            ...
            if (result)
                outStream << "[ graphicsBase ] ERROR Failed to create a debug messenger!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        outStream << "[ graphicsBase ] ERROR Failed to get the function pointer of vkCreateDebugUtilsMessengerEXT!\n";
        return VK_RESULT_MAX_ENUM;
    }
    ...
    static void** SetPNext(void*& pBegin, void* pNext, bool allowDuplicate = false)
    {
        ...
        // 将 lambda 递归调用改为显式捕获所需变量
        auto SetPNext_Internal = [](auto&& self, void*& pBegin, void* pNext, bool allowDuplicate) -> void**
        {
            if (pBegin == pNext)
                return nullptr;
            if (pBegin)
            {
                ...
                if (!allowDuplicate &&
                    ...
                    return self(self, reinterpret_cast<vkStructureHead*>(pBegin)->pNext, pNext, allowDuplicate);
            }
            else
            {
                return &(pBegin = pNext);
...
public:
    ...
    VkResult WaitIdle() const
    {
        ...
        if (result)
            outStream << "[ graphicsBase ] ERROR Failed to wait for the device to be idle!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    {
        std::cout << "used instance layer: " << layerName << std::endl;
    ...
    {
        std::cout << "used instance ext: " << extensionName << std::endl;
    ...
    result_t CreateInstance(VkInstanceCreateFlags flags = 0)
    {
        ...
        arrayRef<const char*> layersToCheck;
        CheckInstanceLayers(layersToCheck);

        arrayRef<const char*> extensionsToCheck;
        CheckInstanceExtensions(extensionsToCheck, nullptr);

        for (auto i :instanceLayers) {
            std::cout << "used layer: " << i << std::endl;
        }

        for (auto i :instanceExtensions) {
            std::cout << "used extension: " << i << std::endl;
        }
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to create a vulkan instance! Error code:" << int32_t(result) << std::endl;
            return result;
        }

        outStream << "Vulkan API Version: " << VK_VERSION_MAJOR(apiVersion) << "." << VK_VERSION_MINOR(apiVersion) << "." << VK_VERSION_PATCH(apiVersion)<<std::endl;
        if constexpr (ENABLE_DEBUG_MESSENGER)
        {
    ...
    result_t CheckInstanceLayers(arrayRef<const char*> layersToCheck) const
    {
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get the count of instance layers!\n";
            return result;
        }
        if (layerCount)
        {
            ...
            {
                outStream << "[ graphicsBase ] ERROR Failed to enumerate instance layer properties!Error code:" << int32_t(result) << std::endl;
                return result;
            }

            for (auto& i : availableLayers)
            {
                std::cout << "support layer: " << i.layerName << std::endl;
            }
    ...
    result_t CheckInstanceExtensions(arrayRef<const char*> extensionsToCheck, const char* layerName) const
    {
        ...
        {
            layerName ? outStream << "[ graphicsBase ] ERROR Failed to get the count of instance extensions! Layer name:" << layerName : outStream << "[ graphicsBase ] ERROR Failed to get the count of instance extensions!";
            return result;
        }
        if (extensionCount)
        {
            ...
            {
                outStream << "[ graphicsBase ] ERROR Failed to enumerate instance extension properties!Error code:" << int32_t(result) << std::endl;
                return result;
            }

            for (auto& i : availableExtensions)
            {
                std::cout << "support extension: " << i.extensionName << std::endl;
            }
    ...
    result_t GetPhysicalDevices()
    {
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get the count of physical devices!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        if (!deviceCount)
            outStream << "[ graphicsBase ] ERROR Failed to find any physical device supports vulkan!\n",
                abort();
        availablePhysicalDevices.resize(deviceCount);
        VkResult result = vkEnumeratePhysicalDevices(instance, &deviceCount, availablePhysicalDevices.data());
        if (result)
            outStream << "[ graphicsBase ] ERROR Failed to enumerate physical devices!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t CreateDevice(VkDeviceCreateFlags flags = 0)
    {
        ...
        if (result)
        {
            outStream << "[ graphicsBase ] ERROR Failed to create a logical device!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        if (queueFamilyIndex_graphics != VK_QUEUE_FAMILY_IGNORED)
        ...
        outStream << "Renderer: " << physicalDeviceProperties.properties.deviceName << std::endl;
    ...
    result_t CheckDeviceExtensions(arrayRef<const char*> extensionsToCheck, const char* layerName = nullptr) const
    {
        ...
        {
            layerName ? outStream << "[ graphicsBase ] ERROR Failed to get the count of device extensions! Layer name:" << layerName : outStream << "[ graphicsBase ] ERROR Failed to get the count of device extensions! ";
            return result;
        }
        if (extensionCount)
        {
            ...
            {
                outStream << "[ graphicsBase ] ERROR Failed to enumerate device extension properties!Error code:" << int32_t(result) << std::endl;
                return result;
            }
            for (auto& i : extensionsToCheck)
            {
    ...
    result_t GetSurfaceFormats()
    {
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get the count of surface formats!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        if (!surfaceFormatCount)
            outStream << "[ graphicsBase ] ERROR Failed to find any supported surface format!\n",
                abort();
    ...
    result_t CreateSwapchain(bool limitFrameRate = true, VkSwapchainCreateFlagsKHR flags = 0)
    {
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get physical device surface capabilities!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        // Set image count
        ...
        swapchainCreateInfo.imageExtent =
            surfaceCapabilities.currentExtent.width == -1 ? VkExtent2D{
        ...
        if (surfaceCapabilities.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT)
            ...
            outStream << "[ graphicsBase ] WARNING VK_IMAGE_USAGE_TRANSFER_DST_BIT isn't supported!\n";
        ...
        if (!swapchainCreateInfo.imageFormat)
                ...
                SetSurfaceFormat({VK_FORMAT_B8G8R8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR}))
            {
                ...
                outStream << "[ graphicsBase ] WARNING Failed to select a four-component UNORM surface format!\n";
            }
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get the count of surface present modes!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        if (!surfacePresentModeCount)
            outStream << "[ graphicsBase ] ERROR Failed to find any surface present mode!\n",
                abort();
        ...
        {
            outStream << "[ graphicsBase ] ERROR Failed to get surface present modes!Error code:" << int32_t(result) << std::endl;
            return result;
        }
    ...
    result_t RecreateSwapchain()
    {
        ...
        if (result)
        {
            outStream << "[ graphicsBase ] ERROR Failed to wait for the queue to be idle!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        ...
        while (VkResult result = vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, semaphore_imageIsAvailable, VK_NULL_HANDLE, &currentImageIndex))
            switch (result)
            {
                ...
                case VK_ERROR_OUT_OF_DATE_KHR:
                    ...
                    break;
                default:
                    outStream << "[ graphicsBase ] ERROR Failed to acquire the next image!Error code:" << int32_t(result) << std::endl;
                    return result;
            }
        return VK_SUCCESS;
    }
    ...
    result_t SubmitCommandBuffer_Graphics(VkSubmitInfo& submitInfo, VkFence fence = VK_NULL_HANDLE) const
    {
        ...
        if (result)
            outStream << "[ graphicsBase ] ERROR Failed to submit the command buffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
                                              ...
                                              VkSemaphore semaphore_renderingIsOver = VK_NULL_HANDLE, VkSemaphore semaphore_ownershipIsTransfered = VK_NULL_HANDLE, VkFence fence = VK_NULL_HANDLE) const
    {
        ...
        if (result)
            outStream << "[ graphicsBase ] ERROR Failed to submit the presentation command buffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t PresentImage(VkPresentInfoKHR& presentInfo)
    {
        ...
        switch (VkResult result = vkQueuePresentKHR(queue_presentation, &presentInfo))
        {
            ...
            case VK_ERROR_OUT_OF_DATE_KHR:
                return RecreateSwapchain();
            default:
                outStream << "[ graphicsBase ] ERROR Failed to queue the image for presentation!Error code:" << int32_t(result) << std::endl;
                return result;
        }
    }
...
public:
    ...
    result_t Create(VkSemaphoreCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ semaphore ] ERROR Failed to create a semaphore!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t Wait() const
    {
        ...
        if (result)
            outStream << "[ fence ] ERROR Failed to wait for the fence!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Reset() const
    {
        ...
        if (result)
            outStream << "[ fence ] ERROR Failed to reset the fence!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Status() const
    {
        ...
        if (result < 0)
            outStream << "[ fence ] ERROR Failed to get the status of the fence!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkFenceCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ fence ] ERROR Failed to create a fence!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t Set() const
    {
        ...
        if (result)
            outStream << "[ event ] ERROR Failed to singal the event!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Reset() const
    {
        ...
        if (result)
            outStream << "[ event ] ERROR Failed to unsingal the event!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Status() const
    {
        ...
        if (result < 0)
            outStream << "[ event ] ERROR Failed to get the status of the event!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkEventCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ event ] ERROR Failed to create a event!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t MapMemory(void*& pData, VkDeviceSize size, VkDeviceSize offset = 0) const
    {
        ...
        {
            outStream << "[ deviceMemory ] ERROR Failed to map the memory!Error code:" << int32_t(result) << std::endl;
            return result;
        }
        if (!(memoryProperties & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT))
        {
            ...
            {
                outStream << "[ deviceMemory ] ERROR Failed to flush the memory!Error code:" << int32_t(result) << std::endl;
                return result;
            }
        }
        return VK_SUCCESS;
    ...
    result_t Allocate(VkMemoryAllocateInfo& allocateInfo)
    {
        ...
        {
            outStream << "[ deviceMemory ] ERROR Invalid memory type index!\n";
            return VK_RESULT_MAX_ENUM;
        }
        allocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        if (VkResult result = vkAllocateMemory(graphicsBase::Base().Device(), &allocateInfo, nullptr, &handle))
        {
            outStream << "[ deviceMemory ] ERROR Failed to allocate memory!Error code:" << int32_t(result) << std::endl;
            return result;
        }
...
public:
    ...
    result_t BindMemory(VkDeviceMemory deviceMemory, VkDeviceSize memoryOffset = 0) const
    {
        ...
        if (result)
            outStream << "[ buffer ] ERROR Failed to attach the memory!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkBufferCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ buffer ] ERROR Failed to create a buffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t BindMemory(VkDeviceMemory deviceMemory, VkDeviceSize memoryOffset = 0) const
    {
        ...
        if (result)
            outStream << "[ image ] ERROR Failed to attach the memory!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkImageCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ image ] ERROR Failed to create an image!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkBufferViewCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ bufferView ] ERROR Failed to create a buffer view!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t Create(VkImageViewCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ imageView ] ERROR Failed to create an image view!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t Create(VkSamplerCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ sampler ] ERROR Failed to create a sampler!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkShaderModuleCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ shader ] ERROR Failed to create a shader module!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(const char* filepath /*reserved for future use*/)
    {
        ...
        if (!file)
        {
            outStream << "[ shader ] ERROR Failed to open the file:" << filepath;
...
public:
    ...
    result_t Create(VkDescriptorSetLayoutCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ descriptorSetLayout ] ERROR Failed to create a descriptor set layout!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkPipelineLayoutCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ pipelineLayout ] ERROR Failed to create a pipeline layout!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkGraphicsPipelineCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ pipeline ] ERROR Failed to create a graphics pipeline!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkComputePipelineCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ pipeline ] ERROR Failed to create a compute pipeline!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkRenderPassCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ renderPass ] ERROR Failed to create a render pass!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Create(VkFramebufferCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ framebuffer ] ERROR Failed to create a framebuffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t Begin(VkCommandBufferUsageFlags usageFlags, VkCommandBufferInheritanceInfo& inheritanceInfo) const
    {
        ...
        if (result)
            outStream << "[ commandBuffer ] ERROR Failed to begin a command buffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t End() const
    {
        ...
        if (result)
            outStream << "[ commandBuffer ] ERROR Failed to end a command buffer!Error code:" << int32_t(result) << std::endl;
        return result;
    }
};
...
public:
    ...
    result_t AllocateBuffers(arrayRef<VkCommandBuffer> buffers, VkCommandBufferLevel level = VK_COMMAND_BUFFER_LEVEL_PRIMARY) const
    {
        ...
        if (result)
            outStream << "[ commandPool ] ERROR Failed to allocate command buffers!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkCommandPoolCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ commandPool ] ERROR Failed to create a command pool!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t AllocateSets(arrayRef<VkDescriptorSet> sets, arrayRef<const VkDescriptorSetLayout> setLayouts) const
    {
        if (sets.Count() != setLayouts.Count())
            if (sets.Count() < setLayouts.Count())
            {
                outStream << "[ descriptorPool ] ERROR For each descriptor set, must provide a corresponding layout!\n";
                return VK_RESULT_MAX_ENUM;
            }
            else
                outStream << "[ descriptorPool ] WARNING Provided layouts are more than sets!\n";
        ...
        if (result)
            outStream << "[ descriptorPool ] ERROR Failed to allocate descriptor sets!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkDescriptorPoolCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ descriptorPool ] ERROR Failed to create a descriptor pool!Error code:" << int32_t(result) << std::endl;
        return result;
    }
...
public:
    ...
    result_t GetResults(uint32_t firstQueryIndex, uint32_t queryCount, size_t dataSize, void* pData_dst, VkDeviceSize stride, VkQueryResultFlags flags = 0) const
    {
        ...
        if (result)
            result > 0 ? outStream << "[ queryPool ] WARNING Not all queries are available!Error code:" << int32_t(result) << std::endl : outStream << "[ queryPool ] ERROR Failed to get query pool results!Error code:" << int32_t(result) << std::endl;
        return result;
    }
    ...
    result_t Create(VkQueryPoolCreateInfo& createInfo)
    {
        ...
        if (result)
            outStream << "[ queryPool ] ERROR Failed to create a query pool!Error code:" << int32_t(result) << std::endl;
        return result;
    }
